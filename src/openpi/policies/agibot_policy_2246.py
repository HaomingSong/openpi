...
class AgibotInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        state = torch.concat(
            [
                data["observation/state/joint"],
                data["observation/state/left_effector"].unsqueeze(0),
                data["observation/state/right_effector"].unsqueeze(0),
            ]
        )
        ...
        if "actions/joint" in data:
            ...
            actions = torch.concat(
                [
                    data["actions/joint"],
                    data["actions/left_effector"].unsqueeze(-1),
                    data["actions/right_effector"].unsqueeze(-1),
                ],
                dim=-1,
            )
